export class OverlayRenderer {
    ...
    renderHud(state, settings) {
      ...
      <div class="row">
        ...
        <span class="badge">target: ${announcement?.roleText || 'none'} ${announcement?.nameText || ''}</span>
        <span class="badge">vc≠focus: ${String(target?.element !== state.keyboardFocus)}</span>
        ${states ? `<span class="badge">${states}</span>` : ''}
        ${lastEvent ? `<span class="badge">event: ${lastEvent.type}</span>` : ''}
      </div>
      <div class="subtitle">${state.speechCurrent?.text || announcement?.spokenText || 'No speech'}</div>
      <div class="help">Shortcuts: ${Object.entries(settings.shortcuts)
            .map(([action, key]) => `${action}:${key}`)
            .join(' · ')}</div>
    `;
        if (settings.hudLevel === 'learner')
            return common;
        const queuePreview = [state.speechCurrent, ...state.speechQueue].filter(Boolean).slice(0, 4).map((item) => item?.text).join(' → ');
        const events = state.events.slice(0, 5).map((event) => `<div>${new Date(event.ts).toLocaleTimeString()} ${event.type}: ${event.label}</div>`).join('');
        return `${common}
      <div class="queue">speech queue: ${queuePreview || 'empty'}</div>
      <div class="dim">name source: ${announcement?.debug.nameSource || '-'} · role source: ${announcement?.debug.roleSource || '-'} · description source: ${announcement?.debug.descriptionSource || '-'}</div>
