import type { AnnouncementSnapshot, AnnouncementReason, CursorStates, CursorTarget } from '../shared/types';
...
function shortPath(el: Element): string {
  ...
  const className = (el as HTMLElement).classList.length ? `.${(el as HTMLElement).classList[0]}` : '';
...
function getRole(el: Element): { role: string; source: 'implicit' | 'explicit'; headingLevel?: number } {
  ...
  if (tag === 'a' && (el as HTMLAnchorElement).href) return { role: 'link', source: 'implicit' };
...
function getName(el: Element): { name: string; source: string } {
...
function getStates(el: Element): CursorStates {
  const htmlEl = el as HTMLElement;
  const states: CursorStates = {};
...
function getValueText(el: Element): string | undefined {
  if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) return el.value || undefined;
  if (el instanceof HTMLSelectElement) return el.selectedOptions[0]?.text || undefined;
  return undefined;
}

function getDescription(el: Element): { text?: string; source: string } {
...
function isNavigable(el: Element): boolean {
  ...
  if (tag === 'img' && Boolean((el as HTMLImageElement).alt)) return true;
...
function categoryFor(el: Element, role: string): CursorTarget['category'] {
  const tag = el.tagName.toLowerCase();
...
export function getCursorTargets(root: ParentNode = document): CursorTarget[] {
...
export function computeAnnouncement(target: CursorTarget, reason: AnnouncementReason): AnnouncementSnapshot {
...
export function shouldAutoFocusMode(target: CursorTarget): boolean {
